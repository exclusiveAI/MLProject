...
class OutputLayer(Layer):
    ...
    def backpropagate(self, y_true: np.ndarray):
        ...
        self.error = mps.multiply(loss_btw_output_and_y_true, self.activation_func.derivative(self.nets))

        return mps.multiply(mps.array(np.insert(self.prev.output, 0, 1, axis=-1).T), self.error)
