...
class Layer:
    ...
    def feedforward(self, input):
        ...
        local_input = mps.array(np.insert(input, 0, 1, axis=-1))  # adding bias to input

        self.nets = mps.matmul(local_input, self.weights)  # calculate the net input for current unit
    ...
    def backpropagate(self, **kwargs):
        ...
        previous_output = mps.array(np.insert(self.prev.output, 0, 1, axis=-1))  # adding bias to input

        next_weights = self.next.weights[1:, :].T

        # calculate the product between the error signal and incoming weights from current unit
        self.error = mps.matmul(self.next.error, next_weights)
        self.error = mps.multiply(self.error, self.activation_func.derivative(self.nets))

        res = mps.multiply(previous_output.T, self.error)
